...
def create_faiss_indices(df, text_columns, model):
    ...
    for column in text_columns:
        ...
        if os.path.exists(index_file_path):
            with st.spinner(f"Loading existing index for column '{column}' from {index_file_path}..."):
                index = faiss.read_index(index_file_path)
        else:
            with st.spinner(f"Generating new index for column '{column}'..."):
...
def fetch_tapas_answer(combined_table, query_text):
    ...
    with st.spinner("Getting answer from TAPAS..."):
...
def main():
    ...
    if uploaded_file is not None:
        ...
        for chat in st.session_state.chat_history:
            ...
            st.write(f"**TAPAS Answer:** {chat['answer']}")

        query_text = st.text_input("Ask a question about the data:")
        if query_text:
            st.write(f"Query: {query_text}")
            
            combined_table = retrieve_nearest_neighbors(query_text, indices, text_columns, df, embedding_model, k=5)
            st.write("Combined Table for TAPAS Query:", combined_table)

            answer = fetch_tapas_answer(combined_table, query_text)
            if 'answer' in answer:
                st.write("**TAPAS Answer:**", answer['answer'])
                
                # Store the question and answer in chat history
                st.session_state.chat_history.append({'question': query_text, 'answer': answer['answer']})
            else:
